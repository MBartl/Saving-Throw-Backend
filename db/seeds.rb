...
# #
# # Current call count: 935 (1191 w/ equipment)
# #
...
# z = 1
# 12.times do
#   User.create(username:"test#{z}", name: Faker::Name.name, email:Faker::Internet.email, bio:Faker::Hipster.sentence(4, false, 4), age:rand(33)+17, location:"#{Faker::Address.city}, #{Faker::Address.state_abbr}", password:"test")
#   z += 1
# end
#
#
...
#   i += 1
#   data = JSON.parse(RestClient.get(url + "/classes/#{i}"))
#   player_class = PlayerClass.all.find{|p_class| p_class.name == data['name']}
#   data['proficiency_choices'][0]['from'].each do |choice|
#     proficiency_choice = Proficiency.all.find{|prof| prof.name == choice['name']}
#     ClassProficiencyChoice.create(player_class: player_class, proficiency: proficiency_choice)
#   end
# end
#
#
...
# 414.times do
#   p += 1
#   data = JSON.parse(RestClient.get(url + "/features/#{p}"))
...
# After two years imprisoned with nothing to eat but his own lawn, the ex-elf has escaped and is in dire need of aid. Transformed assassins are looking to grab themselves a mutton dinner, while the only object capable of restoring his opposable thumbs lies in the hands of his former pupil and current nemesis. Fortunately, the wooly wizard has found new allies he can rely on, right?", max_players: rand(6)+6)
#
...
# With food supplies running low and their sorcerer-in-residence nowhere to be found, the village council sends out a desperate plea for brave adventurers to destroy the beasts for good. Is the job as easy as a walk in the woods, or is there more to the Wolves of Welton than mere animal cunning? There's only one way to find out…", max_players: rand(6)+6)
#
...
# Whether your adventurers are meeting an important NPC in private, they’re in need of a quest hook or two to move the adventure forward, or looking to blow off some steam by partying hard and maybe get into a brawl, the Fiery Grog is the place to go.", max_players: rand(6)+6)
#
...
#   new = Character.create(user: user, name: Faker::Games::WorldOfWarcraft.unique.hero, level: rand(12)+3, bio: Faker::Books::Dune.unique.quote, race: Race.all.sample, player_class: player_class, subclass: Subclass.all.select{|x| x.player_class == player_class}.sample)
#
#   # Create AbilityScore
...
#   player_class = PlayerClass.all.sample
#   user = User.all.sample
#   new = Character.create(user: user, name: Faker::Games::ElderScrolls.unique.name, level: rand(12)+3, bio: Faker::Books::Dune.unique.quote, race: Race.all.sample, player_class: player_class, subclass: Subclass.all.select{|x| x.player_class == player_class}.sample)
#
#   # Create AbilityScore
#   ability_score = AbilityScore.create(character: new, strength: rand(9)+8, dexterity: rand(9)+8, constitution: rand(9)+9, intelligence: rand(9)+8, wisdom: rand(9)+8, charisma: rand(9)+8)
#
...
#   theLegendPB = Character.create(user: User.all.sample, name: "Paul Blart the Magnificent", level: 20, bio: "Safety never takes a holiday.", race: Race.all[3], player_class: player_class, subclass: Subclass.all.find {|x| x.player_class == player_class})
#   ability_score = AbilityScore.create(character: theLegendPB, strength: 22, dexterity: 18, constitution: 20, intelligence: 6, wisdom: 24, charisma: 30)
#
