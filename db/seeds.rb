...
# Create DM's, Chats, Messages and Ownerships
Campaign.all.each do |campaign|
  ...
  (rand(4)+1).times do
    user_ids = chat.campaign.characters.map(&:user).map(&:id)
    Message.create(text: Faker::Lorem.sentence(word_count = (rand(15)+1), supplemental = false, random_words_to_add = 0), chat: chat, user_id: user_ids.sample)
  end
...
# # Seed additional messages (after initial seed - for varying timestamps)
#
...
#   (rand(4)+1).times do
#     user_ids = chat.campaign.characters.map(&:user).map(&:id)
#     Message.create(text: Faker::Hipster.sentence(rand(5)+1, false, rand(5)+1), chat: chat, user_id: user_ids.sample)
#   end
# end
